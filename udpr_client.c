...
void run_udpr_client(const struct sockaddr_in* server_addr, char* data, 
    ...
    while (!b_connection_closed && retransmit_iter < MAX_RETRANSMITS) {
        ...
        b_connection_closed = assert_write(bytes_written, sizeof(connection_data), socket_fd, -1, NULL, data);
        if (!b_connection_closed) {
            ...
            if (bytes_read >= 0 || (bytes_read < 0 && errno != EAGAIN)) {
                b_connection_closed = assert_read(bytes_read, sizeof(conacc_pck), socket_fd, -1, NULL, data);
                if (!b_connection_closed) {
                    b_connection_closed = get_connac_pck(&conacc_pck, session_id);
                    if (!b_connection_closed) {
                        // We got CONACC, let's get the fuck out of here.
                        break;
                    }
                }
            }
    ...
    while(data_length > 0 && !b_connection_closed) {
        ...
        while (!b_connection_closed) {
            ...
            if ((bytes_read < 0 && errno != EAGAIN) || bytes_read == 0) { // Will produce error message.
                b_connection_closed = assert_read(bytes_read, sizeof(acc_pck), socket_fd, -1, data_pck, data);
            }
                ...
                if (bytes_read == sizeof(ACC) && acc_pck.pkt_type_id == ACC_TYPE &&
                    ...
                    error("Data rejected");
                        ...
                        acc_pck.session_id == session_id && acc_pck.pkt_nr < pck_number) &&
                        !(bytes_read == sizeof(CONACC) && acc_pck.pkt_type_id == CONACC_TYPE &&
                        acc_pck.session_id == session_id)) {
